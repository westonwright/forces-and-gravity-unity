...
// radius is only used for terrain collider
// find another use for radius on the other types of collider?
public static class ColliderCalculations
{
    public static Vector3 ClosestPointOnCollider(Collider collider, Vector3 to)
    {
        if (collider is BoxCollider)
        {
            return ClosestPointOnCollider((BoxCollider)collider, to);
        }
        else if (collider is SphereCollider)
        {
            return ClosestPointOnCollider((SphereCollider)collider, to);
        }
        else if (collider is CapsuleCollider)
        {
            return ClosestPointOnCollider((CapsuleCollider)collider, to);
        }
        else if (collider is MeshCollider)
        {
            return ClosestPointOnCollider((MeshCollider)collider, to);
        }
        else if (collider is TerrainCollider)
        {
            return ClosestPointOnCollider((TerrainCollider)collider, to);
        }

        return Vector3.zero;
    }

    public static Vector3 ClosestPointOnCollider(Collider collider, Vector3 to, ref Vector3 normal)
    {
        if (collider is BoxCollider)
        {
            return ClosestPointOnCollider((BoxCollider)collider, to, ref normal);
        }
        else if (collider is SphereCollider)
        {
            return ClosestPointOnCollider((SphereCollider)collider, to, ref normal);
        }
        else if (collider is CapsuleCollider)
        {
            return ClosestPointOnCollider((CapsuleCollider)collider, to, ref normal);
        }
        else if (collider is MeshCollider)
        {
            return ClosestPointOnCollider((MeshCollider)collider, to, ref normal);
        }
        else if (collider is TerrainCollider)
        {
            return ClosestPointOnCollider((TerrainCollider)collider, to, ref normal);
        }

        return Vector3.zero;
    }

    public static Vector3 ClosestPointOnCollider(SphereCollider collider, Vector3 to)
    {
        Vector3 p;
    ...
    public static Vector3 ClosestPointOnCollider(SphereCollider collider, Vector3 to, ref Vector3 normal)
    {
        Vector3 p;
    ...
    public static Vector3 ClosestPointOnCollider(BoxCollider collider, Vector3 to)
    {
    ...
    public static Vector3 ClosestPointOnCollider(BoxCollider collider, Vector3 to, ref Vector3 normal)
    {
        // Cache the collider transform
        var ct = collider.transform;
    ...
    // Courtesy of Moodie
    public static Vector3 ClosestPointOnCollider(CapsuleCollider collider, Vector3 to)
    {
    ...
    public static Vector3 ClosestPointOnCollider(CapsuleCollider collider, Vector3 to, ref Vector3 normal)
    {
        Transform ct = collider.transform; // Transform of the collider
    ...
    public static Vector3 ClosestPointOnCollider(MeshCollider collider, Vector3 to)
    {
        MeshColliderKDTree meshColliderKDTree = collider.GetComponent<MeshColliderKDTree>();

        //runs if the MeshKDTree exists, meaning the KDTree has baked data
        if (meshColliderKDTree != null)
        {
            return meshColliderKDTree.ClosestPointOnSurface(to);
        }
        //runs if the MeshKDTree does not exist and must calculate data each frame
        else
        {
            Debug.LogError("Mesh Collider " + collider.gameObject.name + " does not have a MeshColliderKDTree! Closest point can't be calculated!", collider.gameObject);
            return Vector3.zero;
        }
    }

    public static Vector3 ClosestPointOnCollider(MeshCollider collider, Vector3 to, ref Vector3 normal)
    {
        MeshColliderKDTree meshColliderKDTree = collider.GetComponent<MeshColliderKDTree>();
        ...
        if (meshColliderKDTree != null)
        {
            return meshColliderKDTree.ClosestPointOnSurface(to, ref normal);
        }
        //runs if the MeshKDTree does not exist and must calculate data each frame
        else
        {
    ...
    // determine if higher radius is necessary
    public static Vector3 ClosestPointOnCollider(TerrainCollider collider, Vector3 to)//, bool debug = false)
    {
    ...
    public static Vector3 ClosestPointOnCollider(TerrainCollider collider, Vector3 to, ref Vector3 normal)//, bool debug = false)
    {
        // currently always set to zero
        float radius = 0;
